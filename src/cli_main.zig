...
pub fn main() !void {
    ...
    if (args.len < 2) {
        try out.writeAll("usage: rtt-cli COMMAND\n");
        std.process.exit(2);
    }
    if (std.mem.eql(u8, args[1], "privacy-mode") or std.mem.eql(u8, args[1], "help")) {
        try out.writeAll(privacy_notice);
        return;
    }
    if (std.mem.eql(u8, args[1], "identity") and args.len == 3 and std.mem.eql(u8, args[2], "show")) {
        try out.writeAll("rtt-core unavailable (ERR_CORE_UNAVAILABLE)\n");
        std.process.exit(3);
    }
    _ = rrt;
    try out.writeAll("invalid command or unavailable core\n");
    std.process.exit(2);
}
